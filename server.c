...
int bufi[BUFSIZE];
int buf2[BUFSIZE];
ssize_t r;int i=0,k=0;
//reciving bits//
if ((r = recv(new_fd, buf2, sizeof(buf2), 0)) == -1) {
perror("recv");
exit(1);
}
for(i=0;i<BUFSIZE;i++)
	...
	bufi[i]=alaw2linear((int8_t)(0x00FF&(buf2[i]>>8)))<<16|alaw2linear((int8_t)(0x00FF&buf2[i]));
	}
//playback audio//
if (pa_simple_write(saa, bufi, (ssize_t) r, &error) < 0) {
